...
ACTION atomicswap::whentransfer(const eosio::name& from,
    ...
    eosio_assert(quantity.symbol == eosio::symbol("BOS", 4), "only bos token allowed");
    eosio_assert(quantity.is_valid(), "invalid bos token transfer");
    eosio_assert(quantity.amount > 0, "bos amount should be bigger than 0");
    ...
    eosio_assert(!final_balance_str.empty(), "no bos asset amount");
